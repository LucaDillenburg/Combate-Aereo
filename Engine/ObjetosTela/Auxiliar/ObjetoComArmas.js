...
class InfoArma {
	...
	clone() { return new InfoArma(this.infoTiroPadrao, this.freqAtirar, this.indexArmaGiratoria, this.direcaoSairTiro, this.porcPraDentroObj, this.ehTiroDuplo, this.porcTiroCentro, this.porcComecaAtirar, this.atirarDireto, this.funcaoCondicaoAtirar); }
...
class ObjetoComArmas extends ObjetoTela {
	...
	//getters
	get armas() { return this._armas; }
	//para especificos:
	getControladorTiros(i = 0) { return this._armas[i].controlador; }
	get qtdControladoresTiros() { return this._armas.length; }
	...
	static chaveArmaGiratoria(index) { return "armaGiratoria" + index; }
...
class ObjetoComArmas_e_Vida extends ObjetoComArmas {
	...
	get vidaMAX() { return this._vidaMAX; }

	//mudar vida
	colocarMAXVida() { this._vida = this._vidaMAX; }
	zerarVida() { this.mudarVida(-this._vida); }
	mudarVida(qtdMuda) {
		this._vida += qtdMuda;
		if (this._vida < 0)
			this._vida = 0;
