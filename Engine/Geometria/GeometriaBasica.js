...
class Ponto {
	...
	distancia(ponto) { return Operacoes.hipotenusa(ponto.x - this.x, ponto.y - this.y); }
	...
	clone() { return new Ponto(this.x, this.y); }
}
...
class Angulo {
	...
	static get MENOR_180() { return 0; }
	static get MAIOR_180_CIMA() { return 1; }
	static get MAIOR_180_BAIXO() { return 2; }

	//getters
	get valorRad() { return this._anguloRad; }
	get valorGraus() {
	...
	//aux
	static _magnitude(vetor) { return Math.sqrt(vetor.x * vetor.x + vetor.y * vetor.y); }
	static _vezes(vetor1, vetor2) { return (vetor1.x * vetor2.x) + (vetor1.y * vetor2.y); }
	...
	static angRotacaoParaAngCicloTrig(anguloRotacao) { /*NAO MUDAR ISSO!!:*/return anguloRotacao - PI / 2; }
}
...
class Direcao {
	...
	//static getters
	static get Direita() { return 1; }
	static get Esquerda() { return 2; }
	static get Cima() { return 3; }
	static get Baixo() { return 4; }

	//direcoes: horizontal e vertical
	static get HORIZONTAL() { return 5; }
	static get VERTICAL() { return 6; }
	static get HORIZONTAL_E_VERTICAL() { return 7; }
}
...
class Interseccao {
	...
	static xOuYDePontoEstahDentroDirecao(xOuYDePonto, inicio, distancia) { return xOuYDePonto >= inicio && xOuYDePonto <= inicio + distancia; }
	...
	//objParado eh na verdade cada formaGeometrica de ObjParado que o caminho de ObjQuerAndar colidiu
	{
		...
		if (andarProporcional) {
			...
			if (Math.abs(qtdPodeAndarY1) >= Math.abs(qtdPodeAndarY2))
				...
				// 2: a partir de Y
				return {
					x: (qtdPodeAndarY2 * qtdAndarX) / qtdAndarY, // regra de 3 (vai andar em X proporcional de quanto andara em Y)
					y: qtdPodeAndarY2
				};
		} else
...
class Tela {
	...
	static objVaiSair(obj, qtdMudaX, qtdMudaY) { return Tela.objVaiSairEmX(obj, qtdMudaX) || Tela.objVaiSairEmY(obj, qtdMudaY); }

	//OUTROS
	static xParaEstarNoMeio(widthObj) { return (width - widthObj) / 2; }
	static yParaEstarNoMeio(heightObj) { return (height - heightVidaUsuario - heightObj) / 2; }
}

class Operacoes {
	static hipotenusa(cateto1, cateto2) { return Math.sqrt(cateto1 * cateto1 + cateto2 * cateto2); }

	static diagonalQuad(lado) { return lado * Math.sqrt(2) / 2; }

	static ultimoAlgarismo(num) //retorna um numero inteiro
	{ return Math.floor(num % 10); }
	static primAlgoritDpVirgulaEhZero(num) { return Operacoes.ultimoAlgarismo(num * 10) === 0; }
}
...
class Geometria {
	...
	static get COD_QUADRADO() { return 1; }
	static get COD_RETANGULO() { return 2; }
	static get COD_TRIANGULO() { return 3; }
	static get COD_PARALELOGRAMO() { return 4; }
	static get COD_QUADRILATERO() { return 5; }
	static get COD_FORMA_COMPOSTA() { return 10; }
}
